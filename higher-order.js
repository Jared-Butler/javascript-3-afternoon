...
let evenNumbers = mixedNumbers.filter( param => param%2 === 0 );
...
//Code Here
let postTaxPrices = prices.map( param => param*.07+param);
...
//Code Here
let myStrongest = monstersInYourPocket.filter( param => param.CP >= 200)
...
let orderTotals = orders.map( param => param.price*.07+param.price+param.tax );
